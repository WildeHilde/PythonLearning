...
def matchLines(theLineS):
    ...
    result = []
    for l in theLineS:
        if re.match(r'^\d+$', l): # Only digits
            result.append(str(int(int(l)/2)))
        elif re.match(r'^\D+$', l): # Only non
...
def test_matchLines():
    text = ["288", "ABCDEFGH", "1A2B3C4D", "Hello World", "42"]
    assert matchLines(text) == ["144", 'abcdefgh', None, 'hello world', "21"]
