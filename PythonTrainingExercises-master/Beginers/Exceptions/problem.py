...
def raise_if_not_length_four(value):
    # Your code goes here
    if len(value) != 4: raise ValueError('Argument must be length 4, not {0}'.format(len(value)))
    return value
...
def raise_if_not_four_characters(value):
    if len(value) != 4: raise ValueError('Argument must be length 4, not {0}'.format(len(value)))
    if not isinstance(value, str): raise TypeError('Argument must be a string, not {0}'.format(type(value)))
    return value
...
def get_data_two(network):
    ...
    try:
        ...
        print("That didn't go as I hoped")
    finally:
        db.close
    return result
